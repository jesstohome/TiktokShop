...
class Reply extends Backend
{
    ...
    /**
     * Reply模型对象
     * @var \app\admin\model\product\Reply
     */
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\product\Reply;
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            $list = $this->model
                    ->with(['user','merchant','order','product'])
                    ->where($where)
                    ->order($sort, $order)
                    ->paginate($limit);
            ...
            foreach ($list as $row) {
				...
				$row->getRelation('order')->visible(['order_product_id']);
