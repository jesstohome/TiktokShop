...
class Token
{
    ...
    public static function connect(array $options = [], $name = false)
    {
        ...
        if (true === $name || !isset(self::$instance[$name])) {
            $class = false === strpos($type, '\\') ?
                '\\app\\merchant\\library\\token\\driver\\' . ucwords($type) :
                $type;
     ...
     * @return bool
     */
    public static function has($token, $mer_id)
    {
        return self::check($token, $mer_id);
    }
     ...
     * @return bool
     */
    public static function check($token, $mer_id)
    {
        return self::init()->check($token, $mer_id);
    }
     ...
     * @access public
     * @param string   $token   Token标识
     * @param mixed    $mer_id 商户ID
     * @param int|null $expire  有效时间 0为永久
     * @return boolean
     */
    public static function set($token, $mer_id, $expire = null)
    {
        return self::init()->set($token, $mer_id, $expire);
    }
     ...
     * @return boolean
     */
    public static function clear($mer_id = null)
    {
        return self::init()->clear($mer_id);
    }
