...
class Redis extends Driver
{
     ...
     * @return string
     */
    protected function getUserKey($mer_id)
    {
        return $this->options['userprefix'] . $mer_id;
    }
     ...
     * @return bool
     */
    public function set($token, $mer_id, $expire = 0)
    {
        if (is_null($expire)) {
        ...
        if ($expire) {
            $result = $this->handler->setex($key, $expire, $mer_id);
        } else {
            $result = $this->handler->set($key, $mer_id);
        }
        //写入商户关联的token
        $this->handler->sAdd($this->getUserKey($mer_id), $key);
        return $result;
    }
    ...
    public function get($token)
    {
        ...
        $result = ['token' => $token, 'mer_id' => $value, 'expiretime' => $expiretime, 'expires_in' => $expire];
    ...
    public function delete($token)
    {
        ...
        if ($data) {
            ...
            $this->handler->sRem($this->getUserKey($mer_id), $key);
        }
        return true;
    ...
    {
        $keys = $this->handler->sMembers($this->getUserKey($mer_id));
        $this->handler->del($this->getUserKey($mer_id));
