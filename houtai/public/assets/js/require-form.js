define(['jquery', 'bootstrap', 'upload', 'validator', 'validator-lang'], function ($, undefined, Upload, Validator, undefined) {
    var Form = {
        ...
        events: {
            ...
            selectprop: function (form) {
                //绑定fachoose选择附件事件
                if ($(".select_prop", form).size() > 0) {
                    $(".select_prop", form).on('click', function () {
                        var that = this;
                        // 额外参数 &进行拼接
                        var param = $(this).data("param") ? $(this).data("param") : false;

                        var url = $(this).data("url");
                        if (!url) {
                            Layer.msg('没有执行的url参数');
                        }
                        var multiple = $(this).data("multiple");
                        url = url + "?element_id=" + $(this).attr("id") + (param?param:'')  + '&multiple=' + multiple;
                        console.log(url);
                        parent.Fast.api.open(url, __('Choose'), {
                            callback: function (data) {
                                // console.log(data);

                                var button = $("#" + $(that).attr("id"));
                                // console.log(button);
                                var show_ele = $(button).data("show"); // 显示的内容
                                var hide_ele = $(button).data("hide"); // 隐藏的内容
                                var show_column = $(button).data("show_column"); // 要填充到显示的内容字段
                                var hide_column = $(button).data("hide_column"); // 要填充到隐藏的内容字段
                                // console.log(show_column);
                                // console.log(hide_column);
                                // console.log(data[show_column]);
                                // console.log(data[hide_column]);
                                // console.log(data.data[show_column]);
                                // console.log(data.data[hide_column]);
                                if(data.multiple == true){
                                    var show_columns = '';
                                    var hide_columns = '';
                                    $.each(data.data, function(index, value) {
                                        if(index == 0){
                                            show_columns += value[show_column];
                                            hide_columns += value[hide_column];
                                        }else{
                                            show_columns += ',' + value[show_column];
                                            hide_columns += ',' + value[hide_column];
                                        }
                                    ...
                                    $("#" + show_ele).val(show_columns);
                                    $("#" + hide_ele).val(hide_columns);
                                }else{
                                    $("#" + show_ele).val(data.data[show_column]);
                                    $("#" + hide_ele).val(data.data[hide_column]);
                                }
                            }
                        });
                        return false;
