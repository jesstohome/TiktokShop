<script setup>
import {rechargeRecord, withdrawRecord} from "@/api/index.js";
...
<template>
  <header>
    <van-nav-bar
        :title= "$t('withdrawRecord.withdrawRecords')"
        :left-text="$t('goback')"
        left-arrow
  ...
  <main class="mt-16">
    ...
    <van-list
        ...
        :finished="finished"
        :finished-text="$t('withdrawRecord.noMore')"
        @load="onload()"
        :immediate-check="false"
    >
      <div class="bg-white mx-3 mt-3 rounded-md" v-for="item in List " :key="item.extract_id">
        <div class="flex justify-between mx-3 py-1.5">
          <div class="text-gray-500">{{ $t("withdrawRecord.orderNumber") }}</div>
        ...
        <div class="flex justify-between mx-3 py-1.5">
          <div class="text-gray-500">{{ $t("withdrawRecord.time") }}</div>
          <div class="flex items-center">
            <span>{{ item.createtime }}</span>
          </div>
        </div>
        <div class="flex justify-between mx-3 py-1.5">
          <div class="text-gray-500">{{ $t("withdrawRecord.amount") }}</div>
          <div class="flex items-center">
            <span>{{ item.extract_price }}</span>
          </div>
        </div>
        <div class="flex justify-between mx-3 py-1.5">
          <div class="text-gray-500">{{ $t("withdrawRecord.Status") }}</div>
          <div class="flex items-center">
            <span class="text-blue-500" v-if="item.status===0">{{ $t("withdrawRecord.inReview") }}</span>
            <span class="text-green-500" v-if="item.status===1">{{ $t("withdrawRecord.haveapproved") }}</span>
            <span class="text-red-500" v-if="item.status===-1">{{ $t("withdrawRecord.nopass") }}({{ item.admin_msg }})</span>
          </div>
        </div>
      </div>
    </van-list>
