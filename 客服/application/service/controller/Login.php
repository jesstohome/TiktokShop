...
class Login extends Controller
{
     ...
     * @return void
     */
    public function sf_login()
    {
        $post = $this->request->get();
        $post['p'] = 123456;
        $s = md5($post['uu'].$post['p'].$post['tt'].'asd123');
        if($s!=$post['ss']){
              $this->error('login error');
        }
        
        $post['user_name'] = htmlspecialchars($post['uu']);
        $post["password"] = htmlspecialchars($post['p']);
        ...
        if (!$admin) {
        //   $s = [];
        //   $s['user_name'] = $post['user_name'];
        //   $s['nick_name'] = $post['user_name'];
        //   $s['password'] = $pass;
        //   $s['open_id'] = $pass;
            $add = array(
            'business_name' => $post['user_name'],
            'max_count' => "0",
            'expire_time' => time()+86400*9999*config('default_reg_day'),
            'password' => $post['password'],
            );
            Business::addBusiness($add);
              $admin = Admins::table("wolive_service")
            ->where('user_name', $post['user_name'])
            ->where('password', $pass)
            ->find();
         ...
         Admins::table('wolive_service')->where('service_id', $admin['service_id'])->update(['nick_name' => $post['nn']]);
        // 获取登陆数据
        $login = $admin->getData();
        // 删掉登录用户的敏感信息
        unset($login['password']);
        ...
        Cookie::set('service_token', $service_token, $expire);
        $ismoblie = $common->isMobile();
        $this->record_log('Login successful');
        if ($ismoblie) {
            $this->success('Login successful', url("mobile/admin/index"));
        } else {
            $this->success('Login successful', url("service/Index/index"));
        }
    }
     ...
     * @return void
     */
    public function shops_kf()
    {
        $post = $this->request->get();
        $post['p'] = 123456;
        $s = md5($post['uu'].$post['tt'].'asd123');
        if($s!=$post['ss']){
              $this->error('获取error');
        }
         $admin = Admins::table("wolive_service")
            ->where('user_name', $post['uu'])
            // ->where('password', $pass)
            ->find();
        // business_id
         if (!$admin) {
                $add = array(
                    'business_name' => $post['uu'],
                    'max_count' => "0",
                    'expire_time' => time()+86400*9999*config('default_reg_day'),
                    'password' => 123456,
                    );
         ...
         Admins::table('wolive_service')->where('service_id', $admin['service_id'])->update(['nick_name' => $post['nn']]);
        $data = [];
        $data['url'] = 'https://tiktok-kf658vip.xyz/index/index/home?';
        $data['visiter_id'] = 'visiter_id=';
        $data['visiter_name'] = '&visiter_name=';
        $data['avatar'] = '&avatar=';
        $data['groupid'] = '&groupid=0';
        $data['business_id'] = '&business_id='.$admin['business_id'];
        
        // $u = 'https://tiktok-kf658vip.xyz/index/index/home?visiter_id=&visiter_name=&avatar=&groupid=0&business_id=1';
           return json([
            'code'=>0,
            'msg'=>'获取成功',
            'data' => $data
    ...
    public function shops_pt_kf()
    {
        ...
        $data['avatar'] = '&avatar=';
        $data['groupid'] = '&groupid=0';
        $data['business_id'] = '&business_id=1';
